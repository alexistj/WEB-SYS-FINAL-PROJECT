...
if (isset($_POST['reg_user'])) {
  ...
  $age = mysqli_real_escape_string($db, $_POST['username']);
  $email = mysqli_real_escape_string($db, $_POST['email']);
  $gender = mysqli_real_escape_string($db, $_POST['email']);
  ...
  if (count($errors) == 0) {
    ...
    $query = "INSERT INTO mentees (id, name, age, email, location, gender,password) 
          VALUES(SELECT id from member WHERE name='.$name.'AND email='.'$email'.$name', '$age', '$email', '$loaction', '$gender',  '$password')";
